...
class Settings(BaseSettings):
    ...
    def _resolve_paths(self) -> "Settings":
        ...
        # AuralDesk：安装目录（Program Files）普通用户不可写，允许通过环境变量
        # 把日志/凭证/设备信息数据目录重定向到用户可写位置（如 %LOCALAPPDATA%）
        data_root = os.environ.get("AURALDESK_QQAPI_DATA")
        if data_root:
            root = Path(data_root)
            if not Path(self.logging.file_path).is_absolute():
                self.logging.file_path = str(root / "logs" / Path(self.logging.file_path).name)
            if not Path(self.credential.store.path).is_absolute():
                self.credential.store.path = str(root / Path(self.credential.store.path).name)
            if not Path(self.client.device_path).is_absolute():
                self.client.device_path = str(root / Path(self.client.device_path).name)
            return self
        if not Path(self.logging.file_path).is_absolute():
            # 无环境变量时也统一到用户可写目录，避免 Program Files 下生成第二份设备指纹
            # （两份 device.json 指纹不同会导致 QQ 端每次启动都视为新设备）
            fallback = Path(os.environ.get("LOCALAPPDATA", str(Path.home()))) / "AuralDesk" / "qqapi-data"
            if not Path(self.logging.file_path).is_absolute():
                self.logging.file_path = str(fallback / "logs" / Path(self.logging.file_path).name)
            if not Path(self.credential.store.path).is_absolute():
                self.credential.store.path = str(fallback / Path(self.credential.store.path).name)
            if not Path(self.client.device_path).is_absolute():
                self.client.device_path = str(fallback / Path(self.client.device_path).name)
        return self

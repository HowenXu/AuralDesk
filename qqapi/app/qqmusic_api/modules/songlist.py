...
def _build_songlist_oper_param(
    ...
    """构建歌单写操作的最小 JSON 参数."""
    # dirid=201 是「我喜欢的音乐」特殊歌单，写接口按 Web 端 songType=0 匹配；
    # 保留歌曲原始 type 会导致 DelSonglist 静默空操作（取消收藏不生效）。
    normalized = [(song_id, 0 if dirid == 201 else song_type) for song_id, song_type in song_info]
    return {
        "dirId": dirid,
        "tid": tid,
        "bFmtUtf8": True,
        "v_songInfo": [{"songId": song_id, "songType": song_type} for song_id, song_type in normalized],
        "v_songMid": [],
    }

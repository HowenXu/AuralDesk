...
class VersionPolicy:
    ...
    def build_comm(
        ...
        # 加密下载（GetEVkey）等接口需要 psrf 登录字段，SDK 基础 comm 未覆盖，这里补充
        comm.setdefault("psrf_access_token_expiresAt", credential.expired_at or 0)
        if credential.access_token:
            comm["psrf_qqaccess_token"] = credential.access_token
        if credential.openid:
            comm["psrf_qqopenid"] = credential.openid
        if credential.unionid:
            comm["psrf_qqunionid"] = credential.unionid
